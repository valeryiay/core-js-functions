...
function getPolynom(...coefficients) {
  ...
  return (x) => {
    ...
    coefficients.forEach((coeff, i) => {
      result += coeff * x ** (coefficients.length - i - 1);
    });
    return result;
  };
}
...
function logger(func, logFunc) {
  return (...args) => {
    const argumentList = args.map((item) => JSON.stringify(item)).join(',');
    logFunc(`${func.name}(${argumentList}) starts`);
    const result = func(...args);
    logFunc(`${func.name}(${argumentList}) ends`);
    return result;
  };
}
